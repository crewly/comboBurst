...
class $modify(PlayLayer) {
	...
	bool anyActionRunning() {
		for (int i = 0; i <= m_fields->m_loadedCharacters; i++) {
			...
			if (child) {
				if (child->numberOfRunningActions() > 0) { // FIXME: macos doesn't have this
					return true;
				}
			}
		}
		// return m_fields->m_actionRunning;
		return false;
	}

	void animationCallback() {
		m_fields->m_actionRunning = false;
	...
	void charBurst() {
		...
		if (!anyActionRunning()) {
			m_fields->m_actionRunning = true;
			log::info("{}", m_fields->m_actionRunning);
			...
			auto moveInEase = CCEaseBackOut::create(moveIn); // FIXME: macos doesn't have this
			...
			auto moveOutEase = CCEaseBackIn::create(moveOut); // FIXME: macos doesn't have this
			auto fadeOut = CCFadeTo::create(0.5, 0);

			// Spawn the move and fade actions
			auto spawn = CCSpawn::createWithTwoActions(moveInEase, fadeInEase); // FIXME: macos doesn't have this
			auto despawn = CCSpawn::createWithTwoActions(moveOutEase, fadeOut); // FIXME: nor this
